...
- 共享业务路由表由 Template Route Projector 统一维护；业务页面、目录和外链只能注册在 `src/constants/routes.tsx` 的 `XCODEAGENT_BUSINESS_ROUTES_START/END` 标记之间。Page Agent 只创建或修改 `src/pages/<Feature>/` 页面代码，不得直接编辑该受管区域，也不得维护第二份菜单配置。
