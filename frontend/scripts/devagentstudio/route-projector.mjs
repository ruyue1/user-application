...
const ROUTES_START = '  // DEVAGENTSTUDIO_BUSINESS_ROUTES_START';
const ROUTES_END = '  // DEVAGENTSTUDIO_BUSINESS_ROUTES_END';
...
export function writeRoutesAtomically(routesPath, contents) {
  ...
  const temporaryPath = resolve(directory, `.${basename(routesPath)}.devagentstudio-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.tmp`);
  let descriptor;
  try {
